  ...
  <div class="ArticleSection">
    ...
    <div class="article" v-else>
			...
			<ul>
				...
				<li>
					• 作者:
					<router-link :to="{ name:'user',params:{name:post.author.loginname }}">{{ post.author.loginname }}</router-link>
				</li>
			</ul>
...
export default {
  ...
  computed:{
		  	postTab(){ //计算机属性 岁数据变化而变化
		  		var type = this.post.tab.toString(); //获取了来源 从而改变了这个文章的分类
		  		if(type = 'ask'){
		  			return '问答';
		  		};
		  		if(type = 'share'){
        ...
        this.$http({
				  ...
				  .then( (response) => {
				  	if( response.data.success === true ){
				  		this.post = response.data.data; //赋予this.post值
...
解决的话把scoped属性去掉就行了*/
	@import url("../assets/markdown-github.css");/*引用外部的css文件*/
	.ArticleSection {
